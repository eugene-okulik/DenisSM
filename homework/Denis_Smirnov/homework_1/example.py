...
class entries_count:
    def Find_entries(self, log_entries: dict, text: str,
                     unwanted: str, date: str) -> dict:
        if date:
            if date.startswith('..'):
                date = datetime.strptime(date[3:], '%Y-%m-%d %H:%M:%S.%f')
                log_entries = dict(
                    (key, value) for key, value in log_entries.items() if key <= date)
                ...
                date_1 = datetime.strptime(
                    date[:date.find('/')], '%Y-%m-%d %H:%M:%S.%f')
                date_2 = datetime.strptime(
                    date[date.find('/') + 1:], '%Y-%m-%d %H:%M:%S.%f')
                log_entries = dict(
                    (key, value) for key, value in log_entries.items() if date_1 <= key <= date_2)
            else:
            ...
            for line in log:
                ...
                try:
                    date = datetime.strptime(
                        possible_date, '%Y-%m-%d %H:%M:%S.%f')  # 2022-01-13 12:34:20.517
                    entries[date] = line
                except ValueError:
                    if date:
                        entries[date] += line
